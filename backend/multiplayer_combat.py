...
def resolve_multiplayer_combat_round(shared_state, participants, round_number):
    ...
    state = copy.deepcopy(shared_state); combat = state.get("combat") or {}; enemy = combat.get("enemy") or {}
    ...
    for person in participants:
        character = copy.deepcopy(person.get("character") or {})
        ...
        primaries = primary_stats_for(world, (character.get("special") or {}).get("Archetype", "")) or list(stats) or ["Strength"]
    ...
    combat["round"] = int(combat.get("round", 1) or 1) + 1; combat.setdefault("log", []).extend({"round": round_number, **event} for event in events)
    state["combat"] = combat
