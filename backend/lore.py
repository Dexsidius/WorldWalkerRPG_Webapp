...
def retrieve_lore(world, query, state=None, limit=5):
    entries = [dict(entry, source=entry.get("source", "Built-in Worldwalker notes")) for entry in BUILTIN_LORE.get(world, BUILTIN_LORE["Custom World"])]
    ...
    if not selected:
        selected = [entry for _, _, entry in ranked[:min(2, limit)]]
    return selected

def format_lore_context(world, query, state=None):
    entries = retrieve_lore(world, query, state)
    if not entries:
        return ""
    lines = ["RETRIEVED OFFLINE LORE NOTES (relevant excerpts; official canon and WORLD RULES outrank uncertain interpretations):"]
    lines.extend(f"- {entry.get('title','Lore')} [{entry.get('source','Source not recorded')}]: {entry.get('text','')}" for entry in entries)
    return "\n".join(lines)
