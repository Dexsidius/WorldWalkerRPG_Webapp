...
const LivingAdventures = (() => {
  ...
  async function showLocation(name,focus=true) {
    ...
    try {
      ...
      const groups=[['Rest and preparation',a=>/^(rest:|prepare|scout|activity:|journey:)/.test(a.id)],['Training',a=>a.id.startsWith('train:')],['Known people',a=>a.id.startsWith('talk:')],['Trade',a=>a.id.startsWith('purchase:')]];
