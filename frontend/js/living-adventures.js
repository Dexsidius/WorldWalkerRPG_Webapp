...
const LivingAdventures = (() => {
  ...
  function timeline(panel,data){
    ...
    const render=()=>{
      ...
      panel.querySelectorAll('[data-canon-plan]').forEach(b=>b.onclick=()=>{if(window.OfflinePlay){closeModal('modal-journal');document.getElementById('workspace-map-tab')?.click();showLocation(b.dataset.canonPlace||APP.state.location);return;}const input=document.getElementById('action-input');const line=`Prepare to investigate ${b.dataset.canonPlan}${b.dataset.canonPlace?' at '+b.dataset.canonPlace:''} before ${b.dataset.canonDate}.`;input.value=input.value.trim()?input.value+'\n'+line:line;closeModal('modal-journal');input.focus();showToast('Preparation added to your draft. No time has passed.','notify');});
