...
window.OfflinePlay = (() => {
  ...
  async function refresh(){
    ...
    try{
      ...
      if(['Naruto','One Piece'].includes(s.world)&&!window.WorldwalkerGraphics?.low())panel.querySelectorAll('[data-activity]').forEach(button=>{const row=rows.find(a=>a.id===button.dataset.activity);if(!row)return;const name=row.label.toLowerCase(),file=/rest|recover/.test(name)?(s.world==='Naruto'?'forest_path.webp':'one_piece_foosha_village.webp'):/survey|explor/.test(name)?'forest_path.webp':/route|market|suppl/.test(name)?'merchant_shop.webp':null;if(file){const img=document.createElement('img');img.src='/assets/generated_scenes/'+file;img.className='dream-activity-art';img.alt='';img.loading='lazy';img.decoding='async';button.prepend(img);}});
